...
def main():
    ...
    # Lưu ảnh upload thủ công theo dạng bytes để không bị stale qua rerun
    ...
    # Bên phải: Upload thủ công (LƯU BYTES)
    with c2:
        ...
        uploaded_files = st.file_uploader(
            ...
            type=["jpg", "png", "jpeg"],
            key=f"manual_uploader_{selected_order}",  # tránh clash state giữa các đơn
        )
        ...
        if st.session_state.manual_files:
            if st.button("🗑️ Xóa ảnh đã upload", use_container_width=True):
                st.session_state.manual_files = {}
                st.rerun()
    ...
    # Tạo danh sách lựa chọn (Dropdown) + object nguồn để ZIP xử lý chắc
    ...
    if st.button("📦 Tải File ZIP", type="primary", use_container_width=True):
        ...
        with zipfile.ZipFile(zip_buffer, "w") as zf:
            # 1) Thêm ảnh theo mapping
            for row_i, row in enumerate(edited_mapping.itertuples(index=False), start=1):
                selection = getattr(row, "Chọn Ảnh")
                src = options_map.get(selection)

                if not src:
                    my_bar.progress(min(row_i / max(total_files, 1), 1.0))
                    continue

                file_name = getattr(row, "Tên File Sẽ Lưu")
            ...
            # 2) Thêm Excel Summary (lấy lại order_data mới nhất sau edit)
